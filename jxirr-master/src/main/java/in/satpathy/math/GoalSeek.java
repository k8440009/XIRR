...
public class GoalSeek {
    ...
    private static final boolean DEBUG_GOAL_SEEK = true ;
    ...
    public static GoalSeekStatus fake_df( GoalSeekFunction f, double x,
        ...
        yl = ((Double) status.returnData).doubleValue() ; // XPNV
        if ( DEBUG_GOAL_SEEK ) {
            log( "==> xl = " + xl + " ; yl =" + yl ) ;
        }

        status = f.f( xr, userData ) ;  //yr, userData ) ; // XNPV
        log("status = f.f( xr, userData ) : "  + status);
        if (status.seekStatus != GoalSeekStatus.GOAL_SEEK_OK) {
            if ( DEBUG_GOAL_SEEK ) {
                log("==> failure at xr") ;
            }
        ...
        yr = ((Double) status.returnData).doubleValue() ; // XNPV
        if ( DEBUG_GOAL_SEEK ) {
            log("==> xr = " + xr + " ; yr = " + yr ) ;
        }
        ...
        log(">>>> yr : "  + yr + " >>>> yl : " + yl);
        log(">>>> xr : "  + xr + " >>>> xl : " + xl);
        dfx = (yr - yl) / (xr - xl) ;	// 기울기 (inclination)
        if ( DEBUG_GOAL_SEEK ) {
            log("inclination ==> " + dfx) ;
        }
    ...
    public static GoalSeekStatus goalSeekNewton(
        ...
        for (iterations = 0; iterations < 100; iterations++) {
            ...
            GoalSeekStatus status;
            if ( DEBUG_GOAL_SEEK ) {
                log("x0 is guess rates");
                log("goalSeekNewton - x0 = " + x0 + ", (iterations = " + iterations + " )" ) ;
            }
            ...
            if ( DEBUG_GOAL_SEEK ) {
                log("x0 is guess rates");
                log("status = f.f(x0, userData ) : " + status);
            }
            if ( status.seekStatus != GoalSeekStatus.GOAL_SEEK_OK )   {
                return status ;
            }
            ...
            if ( DEBUG_GOAL_SEEK ) {
                log("   y0(XNPV value) = " + y0 ) ;
            }

            // x0 (rate), y0(XNPV) <- update
            if (update_data (x0, y0, data) )    {
                return new GoalSeekStatus( GoalSeekStatus.GOAL_SEEK_OK,
                                           new Double(data.root) ) ;
            }
            log(">>>>>>>>>>>>>> check value : df : " + df);
            if ( df != null ) {
                ...
                if ( Math.abs(x0) < 1e-10 ) {
                    if (data.havexneg && data.havexpos)
                        xstep = Math.abs(data.xpos - data.xneg) / 1e6; // 1,000,000
                    else
                        xstep = (data.xmax - data.xmin) / 1e6; // 1,000,000
                }
                else    {
                ...
                // 3.0E-7 -> 3.0 * 10 ^ -7
                log(">>>>>>>>>>>>>>>>>>>> xstep : " + xstep);
                status = fake_df(f, x0, xstep, data, userData) ; // return 값 : 기울기

                log(">>>>>>>>>>>>>>>>> fake_df status: " + status);
            }
            if ( status.seekStatus != GoalSeekStatus.GOAL_SEEK_OK )    {
                return status;
            }

            df0 = ((Double) status.returnData).doubleValue() ; // inclination
            log(">>>>>>>>>>>>>>>>>>>> df0 : " + df0);
